...
class CityModel extends GoBaseModel
{
	...
	public function findAllWithCountries(string $selcols='*', int $limit=null, int $offset = 0) { 
		$sql = 'SELECT t1.'.$selcols.',  t2.country_name AS countries_country_name FROM ' . $this->prefix_table . $this->table . ' t1  LEFT JOIN tbl_countries t2 ON t1.country_code = t2.iso_code'; 
